...
class Main(tk.Tk):
    ...
    def __init__(self):
        ...
        ttk.Label(frame1, text="Value:").grid(row=0, column=0, padx=(0, 8), sticky="w")
        value_entry = ttk.Entry(frame1, width=12)
        value_entry.insert(0, "100")
        value_entry.grid(row=0, column=1, padx=(0, 14), sticky="w")

        ttk.Label(frame1, text="From:").grid(row=0, column=2, padx=(0, 6), sticky="w")
        ...
        ttk.Label(frame1, text="To:").grid(row=0, column=4, padx=(0, 6), sticky="w")
        ...
        convert_button = ttk.Button(frame1,text="Convert", command=lambda: result_label.config(text=f"Result: {round(conv.convert_temperature(float(value_entry.get()), from_var.get(), to_var.get()), 2)} {to_var.get()}"))
